...
def _norm_uuid(uuid: str | None) -> str:
    """Notion UUID는 dash 포함/미포함 모두 가능. 정규화 후 비교."""
...
def _verify_ancestor(
    ...
    for _depth in range(max_depth):
        ...
        if ptype == "page_id":
            ...
            if _norm_uuid(actual) == _norm_uuid(expected_page_id):
                return
            raise RuntimeError(
...
def _insert_sync(
    ...
    if target.get("type") == "table_row":
        ...
        target = client.blocks.retrieve(table_block_id)
        anchor_id = table_block_id

    # §19.2 — ancestor traversal로 처리 페이지 page_id 확인. 실패 = 다른 페이지 block 환각.
    _verify_ancestor(client, target, expected_page_id=parent_id)
