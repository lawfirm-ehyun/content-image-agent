...
1. **데이터 정확성** — 본문에 명시된 숫자/텍스트만. 1자도 의역·추측·생성 금지. 가짜 출처 X.
...
- 여백은 padding 토큰(`--sp-3xl = 64px`)으로 통제. **콘텐츠가 짧으면 카드도 짧게, 콘텐츠가 길면 카드도 길게.**
...
모든 카드 슬롯의 `extracted_data.title`은 **필수**:
- 우선순위 1: 슬롯이 속한 H2/H3 본문 헤딩 그대로
- 우선순위 2: 직전 단락의 핵심 명사구
- 의역·추측 X
- `prompt_review.md`가 title 누락/의역을 본문 일치 검증 항목으로 강제
...
- LLM user_prompt 한계 ~20KB (Windows argv). 초과 시 RuntimeError로 명확히 raise. Phase 1 즉시 적용 항목으로 H2 청크 분할 fallback 신설 예정 (plan §19.8).
...
- **비용 cap 두 단계**: Phase 1 임시 `PER_PAGE_CAP_USD = 1.00` (실측 ~$0.70 반영) → Phase 2 안정화 후 `0.30` 복귀 목표. run-level cap 3.00은 batch 진입점에서 누적 추적 (Phase 2).

## 운영 가드 (plan §19 reference — Phase 1 즉시 / Phase 2 진입 전 분류)

**Phase 1 즉시 적용 (코드 수정 항목)**:
- §19.2 block_id ancestor 검증 — insert 직전 `target.parent` 검증으로 다른 페이지 오삽입 방지
- §19.4 Chart.js / Pretendard 로드 검증 — 캡처 직전 `wait_for_function`
- §19.5 review_input 실패도 log_metadata 호출 — 추적성
- §19.6 슬롯 0개도 로그 1행 (`타입=없음` 옵션 추가 후)
- §19.8 본문 길이 fallback — H2 청크 분할
- §19.9 page-level try/except — test_phase1.py 사용자 친화 메시지
