...
## 페이지 status 룰 (운영 컨벤션 — v1.8.5)
- 완료 시 **성공/실패 무관 항상 `이미지 작업 중`**. 모든 완료 페이지가 사람 검수를 거치므로
  (팀 논의 2026-07-09 "잘 만들어져도 어차피 검수해서 뺀다") 시스템은 `발행 필요`를 자동 부여하지 않음.
- `발행 필요`는 검수자가 확인 후 **수동 승격**하는 값 (Notion select 옵션으로 존속).
- 슬롯별 성공/실패 구분이 필요하면 로그 DB `review_passed` row 조회 (status 컬럼엔 안 드러남).
- `이미지 작업 중` 페이지는 cron이 자동으로 다시 처리하지 않음 (cron 은 `이미지 필요`만 fetch).
  **재처리는 운영자가 상태값을 `이미지 필요`로 되돌리면 트리거** (v1.8.4 — 전부 실패 페이지 한정, 아래 멱등성 룰).
